...
- `only`和`except如果都存在在一个job声明中，则所需引用将会被only和except所定义的分支过滤.
- `only`和`except允许使用正则
- `only`和`except可同时使用。如果only和except在一个job配置中同时存在，则以only为准，跳过except(从下面示例中得出)。
- `only`和`except允许使用特殊的关键字：branches，tags和triggers。
- `only`和`except允许使用指定仓库地址但不是forks的仓库(查看示例3)。
